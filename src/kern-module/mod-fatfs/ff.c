...
)
{
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	//kprintf("The logical drive must be mounted.\n");
	...
	//kprintf("Search FAT partition on the drive. Supports only generic partitionings, FDISK and SFD.\n");
	...
	//kprintf("Following code initializes the file system object\n");
	
	if (LD_WORD(fs->win+BPB_BytsPerSec) != SS(fs)) {	/* (BPB_BytsPerSec must be equal to the physical sector size) */
		kprintf("(BPB_BytsPerSec must be equal to the physical sector size)\n");
		kprintf("%d != %d\n", LD_WORD(fs->win+BPB_BytsPerSec), SS(fs));
	...
	//kprintf("Determine the FAT sub type\n");
