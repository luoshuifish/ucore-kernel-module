 ...
 */
static char*
getAbsolutePath(struct ffs_inode* fin, char* path)
{
#if PRINTFSINFO
    kprintf("[getAbsolutePath]\n");
#endif
	int len = strlen(path);
...
#if PRINTFSINFO
	kprintf("[getAbsolutePath] path = %s\n", ret);
#endif
	return ret;
}

static char *
ffs_lookup_subpath(char *path) {
 ...
 */
static int
ffs_dirent_search_nolock(struct ffs_inode *fin, const char* name) {
    int ret = -E_NOENT;
    DIR dirobject;
	FILINFO fno;
	int result;
	...
	if ((result = f_opendir(&dirobject, fin->path)) != 0) {
		kprintf("ls: opendir failed, %d.\n", result);
		return ret;
	}
	while (1) {
		if (f_readdir(&dirobject, &fno) < 0) {
			kprintf("ls: readdir failed.\n");
			break;
		}
 ...
 */
static int
ffs_lookup_once(struct ffs_fs *ffs, struct ffs_inode *fin, TCHAR *name, struct inode **node_store) {
#if PRINTFSINFO
	kprintf("[ffs_lookup_once] path = %s, name = %s\n", fin->path, name);
#endif
	int ret = -E_NOENT;
    if (!strcmp(name, ".")) {
        ret = 0;
 ...
 */
static struct inode *
lookup_ffs_nolock(struct ffs_fs *ffs, TCHAR* path, struct ffs_inode* parent) {
...
#if PRINTFSINFO
	kprintf("[lookup_ffs_nolock] path = %s\n", path);
#endif
	struct inode *node;
...
#if PRINTFSINFO
			kprintf("[lookup_ffs_nolock] find defined node, path = %s, value = %d\n", path, hashValue);
#endif			
 ...
 */
static int
ffs_create_inode(struct ffs_fs *ffs, struct ffs_disk_inode* din, const char *path, struct inode** node_store, struct ffs_inode* parent) {
	struct inode* node;
 ...
 */
static void
ffs_set_links(struct ffs_fs *ffs, struct ffs_inode* fin) {
	struct ffs_inode_list* head = ffs->inode_list;
 ...
 */
static void
ffs_remove_links(struct ffs_fs *ffs, struct ffs_inode* fin) {
 ...
 */
int
ffs_load_inode(struct ffs_fs *ffs, struct inode **node_store, TCHAR *path, struct ffs_inode* parent) {
#if PRINTFSINFO    
	kprintf("[ffs_load_inode] path = %s\n", path);
#endif
	int ret = -E_NO_MEM;
	struct inode* node;
	...
	if (strcmp(path, "0:/")) {
		...
		if ((result = f_stat(actualPath, &fno)) != FR_OK) {
			kprintf("[ffs_load_inode] get stat failed, f_stat %s %d\n", absPath, result);
			kfree(absPath);
			goto failed_cleanup_din;
			return ret;
		} else {
#if PRINTFSINFO
			kprintf("[ffs_load_inode] get stat success, f_stat %s", absPath);
#endif
		}
		kfree(absPath);
 ...
 */
static int
ffs_opendir(struct inode *node, uint32_t open_flags) {
    return 0;
	
	kprintf("[ffs_opendir]\n");
	
	struct ffs_inode* fin = vop_info(node, ffs_inode);
	kprintf("[ffs_opendir], path = %s\n", fin->path);
	int ret = 0;
    fin->din->entity.dir = kmalloc(sizeof(struct DIR));
	if ((ret = f_opendir(fin->din->entity.dir, fin->path)) != FR_OK) {
		kprintf("[ffs_opendir] failed, ret = %d\n", ret);
		return -E_TIMEOUT;
	}
	return ret;
 ...
 */
static int
ffs_openfile(struct inode *node, uint32_t open_flags) {
#if PRINTFSINFO
	kprintf("[ffs_openfile], open_flags = %d\n", open_flags);
#endif
	struct ffs_inode* fin = vop_info(node, ffs_inode);
	BYTE mode;
...
#if PRINTFSINFO
        kprintf("[ffs_openfile], no memory for file\n");
#endif
        return -E_NO_MEM;
    }
	if ((ret = f_open(fin->din->entity.file, fin->path, mode)) != FR_OK) {
#if PRINTFSINFO
		kprintf("[ffs_openfile], fin->path = %s, result = %d\n", fin->path, ret);
#endif
		return -E_INVAL;
	}
    fin->din->size = fin->din->entity.file->fsize;
#if PRINTFSINFO
	kprintf("[ffs_openfile], open %s success, mode = %d\n", fin->path, mode);
#endif
	return 0;
}
...
ffs_close(struct inode *node) {
	...
	if ((result = f_close(fin->din->entity.file)) != FR_OK) {
		kprintf("[ffs_close] close %s failed for %d\n", fin->path, result);
	}
 ...
 */
static int
ffs_closedir(struct inode *node) {
    return 0;
    
    kprintf("[ffs_closedir]\n");
	struct ffs_inode *fin = vop_info(node, ffs_inode);
 ...
 */
static int
ffs_read(struct inode* node, struct iobuf* iob) {
#if PRINTFSINFO
	kprintf("[ffs_read]\n");
#endif
	struct ffs_inode *fin = vop_info(node, ffs_inode);
...
#if PRINTFSINFO
    kprintf("[ffs_read], io_offset = %d, io_len = %d, io_resid = %d\n", iob->io_offset, iob->io_len, iob->io_resid);
#endif
	UINT rc = 0;
	...
	if ((result = f_read(temp, buf, iob->io_len, &rc)) != FR_OK) {
		kprintf("[ffs_read], f_read failed, result = %d\n", result);
	}
	else {
		int i;
 ...
 */
static int
ffs_write(struct inode *node, struct iobuf *iob) {
#if PRINTFSINFO
	kprintf("[ffs_write]\n");
#endif
	int ret = -E_BUSY;
 ...
 */
static int
ffs_fstat(struct inode *node, struct stat* stat) {
#if PRINTFSINFO
	kprintf("[ffs_fstat]\n");
#endif
    int ret = -E_INVAL;
...
ffs_fsync(struct inode* node) {
	...
	if (fin->dirty) {
		...
		if((result = f_sync(fin->din->entity.file)) != FR_OK) {
			kprintf("[ffs_fsync] sync %s failed for %d\n", fin->path, result);
			fin->dirty = 1;
			ret = -E_INVAL;
		}
	}
 ...
 */
static int
ffs_dirent_create_inode(struct ffs_fs* ffs, struct ffs_inode* fin, uint16_t type, struct inode **node_store, const char* name) {
    struct ffs_disk_inode* din;
    if ((din = kmalloc(sizeof(struct ffs_disk_inode))) == NULL) {
        return -E_NO_MEM;
    }
 ...
 */
static int
ffs_mkdir(struct inode *node, const char* name) {
#if PRINTFSINFO
	kprintf("[ffs_mkdir]\n");
#endif
    ...
    if ((ret = f_mkdir(lnkfin->path)) != FR_OK) {
        kprintf("[ffs_mkdir] error, result = %d\n", ret);
        return -E_NO_MEM;
    }
...
ffs_link(struct inode *node, const char *name, struct inode *link_node) {
    ...
    return -E_NOTCOMPLETED;
    
	int ret = -E_NOTCOMPLETED;
 ...
 */
static struct ffs_inode*
findNode(struct ffs_fs* ffs, const char* name, uint32_t hashno) {
    struct ffs_inode_list* head = ffs->inode_list;
	while (head->next != NULL) {
		head = head->next;
		struct ffs_inode* fin = head->f_inode;
 ...
 */
static int
ffs_rename(struct inode *node, const char *name, struct inode *new_node, const char *new_name) {
#if PRINTFSINFO
	kprintf("[ffs_rename]\n");
#endif
	...
	int ret;
    struct ffs_fs *ffs = fsop_info(vop_fs(node), ffs);
	struct ffs_inode *fin = vop_info(node, ffs_inode), *newfin = vop_info(new_node, ffs_inode);
 ...
 */
static int 
ffs_namefile(struct inode *node, struct iobuf *iob) {
#if PRINTFSINFO
	kprintf("[ffs_namefile]\n");
#endif
    if (iob->io_resid <= 2) {
        return -E_NO_MEM;
    }
...
#if PRINTFSINFO
        kprintf("[ffs_namefile], fin->path = %s\n", fin->path);
#endif
        --alen;
    }
    else {
 ...
 */
static int
ffs_getdirentry(struct inode *node, struct iobuf *iob) {
#if PRINTFSINFO
	kprintf("[ffs_getdirentry]\n");
#endif
    ...
    struct ffs_inode* fin = vop_info(node, ffs_inode);
    if ((result = f_opendir(&dirobject, fin->path)) != 0) {
        kprintf("[ffs_getdirentry] opendir error = %d, path = %s\n", result, fin->path);
        return -E_INVAL;
    }
    
    while (1) {
        if ((result = f_readdir(&dirobject, &fno)) != 0) {
            kprintf("[ffs_getdirentry] readdir error = %d\n", result);
            return -E_INVAL;
        }
 ...
 */
static int
ffs_reclaim(struct inode *node) {
    return 0;
	
    struct ffs_fs* ffs = fsop_info(vop_fs(node), ffs);
	struct ffs_inode* fin = vop_info(node, ffs_inode);
	kprintf("[ffs_reclaim], path = %s\n", fin->path);
	...
	if (fin->dirty) {
		if ((ret = vop_fsync(node)) != 0) {
			kprintf("[ffs_reclaim] sync failed\n");
			goto failed;
		}
	}
	...
	kprintf("[ffs_reclaim] reclaim success\n");
	return 0;
 ...
 */
static int
ffs_gettype(struct inode *node, uint32_t *type_store) {
    ...
    switch(din->type) {
        case FFS_TYPE_DIR:
#if PRINTFSINFO
            kprintf("[ffs_gettype] FFS_TYPE_DIR\n");
#endif
			*type_store = S_IFDIR;
			return 0;
		case FFS_TYPE_FILE:
#if PRINTFSINFO
            kprintf("[ffs_gettype] FFS_TYPE_FILE\n");
#endif
			*type_store = S_IFREG;
			return 0;
		case FFS_TYPE_LINK:
#if PRINTFSINFO
            kprintf("[ffs_gettype] FFS_TYPE_LINK\n");
#endif
			*type_store = S_IFLNK;
			return 0;
	}
 ...
 */
static int
ffs_tryseek(struct inode *node, off_t pos) {
#if PRINTFSINFO
	kprintf("[ffs_tryseek]\n");
#endif
	...
	struct ffs_inode *fin = vop_info(node, ffs_inode);
	if (pos > fin->din->size) {
		kprintf("[ffs_tryseek] too big pos: %d\n", pos);
 ...
 */
static int
ffs_truncdir(struct inode *node, off_t len) {
	kprintf("[ffs_truncdir]\n");
	int ret = -E_NOTCOMPLETED;
	return ret;
}
...
static int
ffs_truncfile(struct inode *node, off_t len) {
    struct ffs_inode *fin = vop_info(node, ffs_inode);
	struct ffs_disk_inode *din = fin->din;
	
#if PRINTFSINFO
	kprintf("[ffs_truncfile]\n");
    kprintf("[ffs_truncfile] path = %s, len = %d\n", fin->path, len);
#endif
	...
	if ((ret = f_truncate(din->entity.file)) != FR_OK) {
		kprintf("[ffs_truncfile], result = %d\n", ret);
		ret = -E_TOO_BIG;
	}
	else {
		ret = 0;
 ...
 */
static int
ffs_create(struct inode *node, const char *name, bool excl, struct inode **node_store) {
	if (strlen(name) > FFS_MAX_FNAME_LEN) {
		return -E_TOO_BIG;
	}
	if (!strcmp(name, ".") || !strcmp(name, "..")) {
    ...
    if ((ret = ffs_dirent_search_nolock(fin, name)) != -E_NOENT) {
        ...
        if (!excl) {
            ...
            if (vop_info(link_node, ffs_inode)->din->type == FFS_TYPE_FILE) {
                goto out;
            }
            vop_ref_dec(link_node);
        }
 ...
 */
static int
ffs_unlink(struct inode *node, const char *name) {
#if PRINTFSINFO
	kprintf("[ffs_unlink]\n");
#endif    
    if (strlen(name) > FFS_MAX_FNAME_LEN) {
    ...
    if (lnkfin->dirty) {
		if ((ret = vop_fsync(link_node)) != 0) {
			kprintf("[ffs_unlink] sync failed\n");
		}
	}
    ...
    if ((result = f_unlink(lnkfin->path)) != FR_OK) {
        kprintf("[ffs_unlink] unlink failed, error = %d\n", result);
        ret = -E_UNIMP;
    }
    else {
        fin->dirty = 1;
 ...
 */
static int
ffs_lookup(struct inode *node, char *path, struct inode **node_store) {
    struct ffs_fs *ffs = fsop_info(vop_fs(node), ffs);
    assert(*path != '\0' && *path != '/');
    vop_ref_inc(node);
#if PRINTFSINFO
	kprintf("[ffs_lookup], node.path = %s, path = %s\n", vop_info(node, ffs_inode)->path, path);
#endif
...
#if PRINTFSINFO
		kprintf("[ffs_lookup] get subpath=%s\n", subpath);
#endif
...
#if PRINTFSINFO
		kprintf("[ffs_lookup] to lookup once, path=%s\n", path);
#endif
 ...
 */
static int
ffs_lookup_parent(struct inode *node, char *path, struct inode **node_store, char **endp) {
#if PRINTFSINFO
	kprintf("[ffs_lookup_parent]\n");
#endif
    struct ffs_fs *ffs = fsop_info(vop_fs(node), ffs);
    assert(*path != '\0' && *path != '/');
    vop_ref_inc(node);
