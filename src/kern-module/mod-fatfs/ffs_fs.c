...
static int
ffs_do_mount(struct device *dev, struct fs **fs_store) {
...
label_out:
	...
	*/
	...
	if ((result = f_open(fp, "ctest", FA_READ)) != FR_OK) {
		kprintf("[ffs_do_mount], f_open, %d\n", result);
		
	} else {
		kprintf("f_open successful\n");
		BYTE s[4096];
		UINT rc = 0;
		fp->fptr = 0;
