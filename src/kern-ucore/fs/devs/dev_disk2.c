...
static int
disk2_open(struct device *dev, uint32_t open_flags) {
    return 0;
}

static int
disk2_close(struct device *dev) {
    return 0;
}

static void
disk2_read_blks_nolock(uint32_t blkno, uint32_t nblks) {
    int ret;
    uint32_t sectno = blkno * DISK2_BLK_NSECT, nsecs = nblks * DISK2_BLK_NSECT;
    if ((ret = ide_read_secs(DISK2_DEV_NO, sectno, disk2_buffer, nsecs)) != 0) {
        panic("disk2: read blkno = %d (sectno = %d), nblks = %d (nsecs = %d): 0x%08x.\n",
                blkno, sectno, nblks, nsecs, ret);
    }
}

static void
disk2_write_blks_nolock(uint32_t blkno, uint32_t nblks) {
    int ret;
    uint32_t sectno = blkno * DISK2_BLK_NSECT, nsecs = nblks * DISK2_BLK_NSECT;
    if ((ret = ide_write_secs(DISK2_DEV_NO, sectno, disk2_buffer, nsecs)) != 0) {
        panic("disk2: write blkno = %d (sectno = %d), nblks = %d (nsecs = %d): 0x%08x.\n",
                blkno, sectno, nblks, nsecs, ret);
    }
}

static int
disk2_io(struct device *dev, struct iobuf *iob, bool write) {
    ...
    while (resid != 0) {
        ...
        if (write) {
            ...
            assert(copied != 0 && copied <= resid && copied % DISK2_BLKSIZE == 0);
...
static int
disk2_ioctl(struct device *dev, int op, void *data) {
    return -E_UNIMP;
}

static void
disk2_device_init(struct device *dev) {
