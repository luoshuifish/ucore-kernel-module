...
int
main(void) {
    int fd = safe_open("disk1:filetest", O_RDWR | O_TRUNC);
    struct stat *stat = safe_fstat(fd);
    assert(stat->st_size == 0 && stat->st_blocks == 0);

    safe_seek(fd, sizeof(buffer), LSEEK_SET);
    ...
    for (i = 0; i < sizeof(buffer); i ++) {
        assert(buffer[i] == (unsigned char)i + 65);
    }
    /*
    fd = safe_open("disk1:filetest", O_RDWR | O_TRUNC);
    stat = safe_fstat(fd);
    assert(stat->st_size == 0 && stat->st_blocks == 0);
    */
    printf("sfatfs file test pass.\n");
    return 0;
}
